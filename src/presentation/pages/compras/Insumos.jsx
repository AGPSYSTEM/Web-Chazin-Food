...
export function Insumos() {
  ...
  const fetchInsumos = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setInsumosData((Array.isArray(data) ? data : []).map(i => ({
          ...i,
          stockActual: Number(i.stock || 0),
          stockMinimo: Number(i.stockMinimo || 0),
          fechaExpedicion: i.fechaExpedicion ? i.fechaExpedicion.substring(0, 10) : "",
          fechaVencimiento: i.fechaVencimiento ? i.fechaVencimiento.substring(0, 10) : "",
  ...
  const fetchPreparados = async () => {
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        const dataWithCosto = (Array.isArray(data) ? data : []).map(prep => {
          const compList = (prep.componentes || prep.insumos || []).map(c => ({
            idInsumo: c.idInsumo || c.id || c.id_insumo || Math.random(),
            nombre: c.nombre || (c.Insumo ? c.Insumo.nombre : 'Insumo'),
            cantidad: Number(c.cantidad || (c.InsumoPreparado ? c.InsumoPreparado.cantidad : 1)),
            unidadMedida: c.unidadMedida || c.unidad || 'und',
            precioUnitario: Number(c.precioUnitario || c.precio || 0)
          }));
          const costoTotal = compList.reduce((s, c) => s + (c.cantidad * c.precioUnitario), 0);
          return { ...prep, componentes: compList, costoTotal };
        });
  ...
  const fetchDeleted = async () => {
    try {
      ...
      if (resIns.ok) {
        ...
        setDeletedInsumos((Array.isArray(data) ? data : []).map(i => ({
          ...i,
    ...
    try {
      if (editandoPreparadoId !== null) {
        ...
        if (response.ok) {
          ...
          const errData = await response.json().catch(() => ({}));
          notify.error("Error", errData.message || "No se pudo actualizar el insumo preparado");
        }
      } else {
        const response = await fetch('http://localhost:5000/api/insumos-preparados', {
          method: 'POST',
        ...
        if (response.ok) {
          ...
          const errData = await response.json().catch(() => ({}));
          notify.error("Error", errData.message || "No se pudo crear el insumo preparado");
        }
      }
    } catch (error) {
      console.error(error);
  ...
  const openEdit = (i) => {
    ...
    setEditInsumo({
      ...
      stockActual: i.stockActual !== undefined ? i.stockActual : (i.stock !== undefined ? i.stock : 0),
      stockMinimo: i.stockMinimo !== undefined ? i.stockMinimo : 0,
      fechaExpedicion: i.fechaExpedicion || "",
      fechaVencimiento: i.fechaVencimiento || "",
      unidadMedida: i.unidadMedida || "kg",
      precioUnitario: i.precioUnitario !== undefined ? i.precioUnitario : 0,
  ...
  const handleSaveNewInsumo = async () => {
    ...
    const stockNum = newInsumo.stockActual !== "" && newInsumo.stockActual !== undefined ? parseInt(newInsumo.stockActual) : NaN;
    const stockMinNum = newInsumo.stockMinimo !== "" && newInsumo.stockMinimo !== undefined ? parseInt(newInsumo.stockMinimo) : 0;
    const precioNum = newInsumo.precioUnitario !== "" && newInsumo.precioUnitario !== undefined ? parseFloat(newInsumo.precioUnitario) : NaN;
    ...
    try {
      const response = await fetch('http://localhost:5000/api/insumos', {
        ...
        body: JSON.stringify({
          ...
          fechaExpedicion: newInsumo.fechaExpedicion || null,
          fechaVencimiento: newInsumo.fechaVencimiento || null,
          unidadMedida: newInsumo.unidadMedida || 'kg',
          precioUnitario: precioNum,
          idProveedor: newInsumo.idProveedor ? parseInt(newInsumo.idProveedor) : null,
      ...
      if (response.ok) {
        await fetchInsumos();
        setShowModal(false);
        setNewInsumo({ nombre: "", idCategoriaInsumo: "", stockActual: "", stockMinimo: "", fechaExpedicion: "", fechaVencimiento: "", unidadMedida: "kg", precioUnitario: "", idProveedor: "", descripcion: "", fichaTecnica: null });
        setFormSubmitted(false);
        registrarEvento("crear", newInsumo.nombre, `Se agregó un nuevo insumo al inventario: ${newInsumo.nombre}`);
        notify.success("Insumo creado", "El insumo se agregó correctamente");
      } else {
        const errData = await response.json().catch(() => ({}));
        notify.error("Error", errData.message || "No se pudo guardar el insumo");
      }
    } catch (error) {
      console.error(error);
  ...
  const handleSaveEditInsumo = async () => {
    ...
    const targetId = editInsumo?.idInsumo || editInsumo?.id || selectedInsumo?.idInsumo || selectedInsumo?.id;
    const stockNum = editInsumo?.stockActual !== "" && editInsumo?.stockActual !== undefined ? parseInt(editInsumo.stockActual) : NaN;
    const stockMinNum = editInsumo?.stockMinimo !== "" && editInsumo?.stockMinimo !== undefined ? parseInt(editInsumo.stockMinimo) : 0;
    const precioNum = editInsumo?.precioUnitario !== "" && editInsumo?.precioUnitario !== undefined ? parseFloat(editInsumo.precioUnitario) : NaN;
    ...
    try {
      const response = await fetch(`http://localhost:5000/api/insumos/${targetId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          nombre: editInsumo.nombre.trim(),
          idCategoriaInsumo: parseInt(editInsumo.idCategoriaInsumo),
          stock: stockNum,
          stockMinimo: stockMinNum,
          fechaExpedicion: editInsumo.fechaExpedicion || null,
          fechaVencimiento: editInsumo.fechaVencimiento || null,
          unidadMedida: editInsumo.unidadMedida || 'kg',
          precioUnitario: precioNum,
          idProveedor: editInsumo.idProveedor ? parseInt(editInsumo.idProveedor) : null,
      ...
      if (response.ok) {
        ...
        const errData = await response.json().catch(() => ({}));
        notify.error("Error", errData.message || "No se pudo actualizar el insumo");
      }
    } catch (error) {
      console.error(error);
  ...
  const handleDeleteInsumo = async () => {
    ...
    const targetId = selectedInsumo.idInsumo || selectedInsumo.id;
  ...
  const getStockBadge = (stockActual, stockMinimo = 0) => {
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-700/60">
              {currentInsumos.map((insumo) => <tr key={insumo.idInsumo} className="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium ${getStockBadge(insumo.stockActual, insumo.stockMinimo)}`}>
                      {getStockStatus(insumo.stockActual, insumo.stockMinimo)}
                    </span>
                  </td>
                  <td className="px-6 py-4">
          ...
          <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Stock Actual <span className="text-red-500">*</span></label>
                  <input
                    type="number"
                    value={editInsumo?.stockActual !== undefined ? editInsumo?.stockActual : 0}
                ...
                <div>
                  ...
                  <input
                    type="number"
                    value={editInsumo?.stockMinimo !== undefined ? editInsumo?.stockMinimo : 0}
                    onChange={(e) => setEditInsumo({ ...editInsumo, stockMinimo: e.target.value === "" ? "" : parseInt(e.target.value) })}
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Unidad Medida</label>
                  <select
                ...
                <div>
                  ...
                  <input
                    type="number"
                    value={editInsumo?.precioUnitario !== undefined ? editInsumo?.precioUnitario : 0}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Fecha Expedición</label>
                  <input
                    type="date"
                    value={editInsumo?.fechaExpedicion || ""}
                    onChange={(e) => setEditInsumo({ ...editInsumo, fechaExpedicion: e.target.value })}
                    className="w-full px-4 py-2.5 border border-gray-200 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100 rounded-xl text-sm focus:ring-2 focus:ring-red-500 focus:border-transparent outline-none transition"
                  />
                </div>
                <div>
                  ...
                  <input
                    type="date"
                    value={editInsumo?.fechaVencimiento || ""}
                    onChange={(e) => setEditInsumo({ ...editInsumo, fechaVencimiento: e.target.value })}
                    className="w-full px-4 py-2.5 border border-gray-200 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100 rounded-xl text-sm focus:ring-2 focus:ring-red-500 focus:border-transparent outline-none transition"
                  />
                </div>
              </div>
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setEditInsumo({ ...editInsumo, idProveedor: e.target.value ? parseInt(e.target.value) : "" })}
                  className="w-full px-4 py-2.5 border border-gray-200 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100 rounded-xl text-sm focus:ring-2 focus:ring-red-500 focus:border-transparent outline-none transition"
                >
                  <option value="">Seleccionar proveedor (Sin proveedor)...</option>
              ...
              <div>
                ...
                <textarea
                  value={editInsumo?.descripcion || ""}
                  onChange={(e) => setEditInsumo({ ...editInsumo, descripcion: e.target.value })}
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className={`w-full px-4 py-2.5 border rounded-xl text-sm outline-none transition ${
                      formSubmitted && (!String(newInsumo.stockActual).trim() || parseInt(newInsumo.stockActual) < 0)
                        ? "border-red-500 focus:ring-red-500/50"
                        : "border-gray-200 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100 focus:ring-2 focus:ring-red-500 focus:border-transparent"
                    }`}
                    placeholder="0"
                  />
                  {formSubmitted && !String(newInsumo.stockActual).trim() && (
                    <p className="text-red-500 text-xs mt-1">campo obligatorio*</p>
                  )}
                  {formSubmitted && String(newInsumo.stockActual).trim() && parseInt(newInsumo.stockActual) < 0 && (
                    <p className="text-red-500 text-xs mt-1">campo inválido</p>
                ...
                <div>
                  ...
                  <input
                    ...
                    className={`w-full px-4 py-2.5 border rounded-xl text-sm outline-none transition ${
                      formSubmitted && (!String(newInsumo.precioUnitario).trim() || parseFloat(newInsumo.precioUnitario) < 0)
                        ? "border-red-500 focus:ring-red-500/50"
                        : "border-gray-200 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100 focus:ring-2 focus:ring-red-500 focus:border-transparent"
                    }`}
                    placeholder="$0"
                  />
                  {formSubmitted && !String(newInsumo.precioUnitario).trim() && (
                    <p className="text-red-500 text-xs mt-1">campo obligatorio*</p>
                  )}
                  {formSubmitted && String(newInsumo.precioUnitario).trim() && parseFloat(newInsumo.precioUnitario) < 0 && (
                    <p className="text-red-500 text-xs mt-1">campo inválido</p>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    type="date"
                    value={newInsumo.fechaExpedicion}
                    onChange={(e) => setNewInsumo({ ...newInsumo, fechaExpedicion: e.target.value })}
                    className="w-full px-4 py-2.5 border border-gray-200 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100 rounded-xl text-sm focus:ring-2 focus:ring-red-500 focus:border-transparent outline-none transition"
                  />
                </div>
                <div>
                  ...
                  <input
                    type="date"
                    value={newInsumo.fechaVencimiento}
                    onChange={(e) => setNewInsumo({ ...newInsumo, fechaVencimiento: e.target.value })}
                    className="w-full px-4 py-2.5 border border-gray-200 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100 rounded-xl text-sm focus:ring-2 focus:ring-red-500 focus:border-transparent outline-none transition"
                  />
                </div>
              </div>

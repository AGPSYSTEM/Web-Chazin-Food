...
export function Roles() {
  ...
  const toggleEstado = async (id) => {
    ...
    if (!rolActual) return;
    const nuevoEstadoStr = rolActual.estado === "Activo" ? "Inactivo" : "Activo";
    ...
    try {
      ...
      if (!response.ok) {
        const errData = await response.json();
        throw new Error(errData.message || "Error al actualizar el estado del rol");
      }

      setRoles((prev) => prev.map((r) => r.id === id ? { ...r, estado: nuevoEstadoStr } : r));
      showToast(`Estado del rol "${rolActual.nombre}" actualizado a ${nuevoEstadoStr}`);
    } catch (error) {
      console.error(error);
      alert(error.message || "Error al actualizar el estado del rol");
    }
  };
